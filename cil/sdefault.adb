...
--  This is the GNAT for .NET version of package body Sdefault.

--  This package body provides the .NET implementation of the routines
...
--  the Makefile. In the case of GNAT for .NET, this package body exists
--  because it is non trivial and most important it is installation directory
--  independent which makes its automatic Makefile generation unnecsssary
--  (unlike the regulat GNAT sdefault.adb).

--  GNAT for .NET differs in the way it locates the Ada library directories
--  from the regular GNAT. Assuming it is installed in directory <install-dir>,
--  <install-dir> will contain, amongst others two important directories: bin
--  and lib as shown in the picture below.

--  <install-dir>
--      |
--      +-- bin  [contains all the GNAT executables, such as dotnet-gnatmake]
--      |
--      +-- lib
--      |    |
--      |    +-- dotgnat
--      |    |     |
--      |    |     +-- adainclude [Ada library sources]
--      |    |     |
--      |    |     +-- adalib     [.ali & zipped class file for the above]
--      :    :

--  The user can install GNAT in any <install-dir> but is not allowed to
--  modify the directory hierarchy shown above.

--  To locate directories adainclude and adalib the GNAT executables such
