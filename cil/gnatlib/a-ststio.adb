...
--  This is the .NET/JVM version of the body of Ada.Streams.Stream_IO.
...
package body Ada.Streams.Stream_IO is
   ...
   begin
      --  This call is currently needed by the JGNAT implementation in order to
      --  communicate to GNAT_libc that a stream file being created, which will
      --  be represented using a Java RandomAccessFile.
   ...
   begin
      --  This call is currently needed by the JGNAT implementation in order to
      --  communicate to GNAT_libc that a stream file being opened, which will
      --  be represented using a Java RandomAccessFile.
