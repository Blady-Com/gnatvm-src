...
package body Ada.Sequential_IO is
   ...
   function End_Of_File (File : File_Type) return Boolean is
      function Object_End_Of_File
        (Stream : Interfaces.C_Streams.FILEs)
           return Boolean;
      pragma Import
        (CIL, Object_End_Of_File, "mgnat.adalib.Object_File.object_eof");
   ...
   procedure Read (File : File_Type; Item : out Element_Type) is
      ...
      function Read_Element
        (Stream : Interfaces.C_Streams.FILEs) return System.Address;
      pragma Import
        (CIL, Read_Element, "mgnat.adalib.Object_File.read_element");

      type Elt_Access is access all Element_Type;

      function To_Element is new
        Ada.Unchecked_Conversion (System.Address, Elt_Access);

   begin
      FIO.Check_Read_Status (AP (File));

      Set_Index (File, Index (File));
      Item := To_Element (Read_Element (File.Stream)).all;
   ...
   procedure Write (File : File_Type; Item : Element_Type) is
      ...
      pragma Import
        (CIL, Write_Element, "mgnat.adalib.Object_File.write_element");

      type Elt_Access is access all Element_Type;

      function To_Address is new
        Ada.Unchecked_Conversion (Elt_Access, System.Address);

   begin
      FIO.Check_Write_Status (AP (File));

      Set_Index (File, Index (File));
      Write_Element (File.Stream, To_Address (new Element_Type'(Item)));
      Set_Index (File, Index (File) + 1);
   end Write;
   ...
   procedure Set_Index (File : File_Type; To : long) is
      procedure Set_File_Index
        (Stream : Interfaces.C_Streams.FILEs;
         Index  : long);
      pragma Import
        (CIL, Set_File_Index, "mgnat.adalib.Object_File.set_file_index");
   ...
   function Index (File : File_Type) return long is
      ...
      pragma Import
        (CIL, File_Index, "mgnat.adalib.Object_File.file_index");
