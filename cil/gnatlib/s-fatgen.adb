...
--  The implementation here is specific to .NET and JVM using
--  IEEE floating point (though presently only the implementation of attribute
--  Valid differs from the target-independent version of this package).
--  It must be kept up to date with any corresponding changes made to
--  s-fatgen.adb.
...
package body System.Fat_Gen is
   ...
   function Valid (X : not null access T) return Boolean is
      function C_Valid (X : T) return Boolean;
      pragma Import (C, C_Valid, "__gnat_fl_valid");
      --  C# method using System.Double or System.Float to determine if X
      --  is valid.
   begin
      return C_Valid (X.all);
   end Valid;
   ...
   begin
      --  Current implementation for JGNAT: Not clear how to implement this
      return True;
   end Unaligned_Valid;
