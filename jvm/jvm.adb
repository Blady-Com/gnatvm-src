...
package body JVM is
   ...
   procedure Check_Compare_Type;
   --  Check validity of object on stack and if invalid, raise Invalide_Types
   ...
   is
      ...
      pragma Unreferenced (Init_Method);
   ...
   begin
      ...
      --  A literal value outside the range of type short requires
      --  a pool reference (SIPUSH is the instruction for the largest
      --  push of an immediate integer value).
   ...
   is
      ...
      This   : Local_Var_Id;
      pragma Unreferenced (Exp_Stdcall, This, Delegate, Is_AR_Method);
   ...
   begin
      ...
      --  Relax the matching for Boolean, Byte, Char, and Short
         ...
         --  Allow the top-of-stack class to match any ancestor class
            ...
            if Type_Kind (Stack_Type) /= Class_Kind then
               return False;
            end if;
   ...
   begin
      ...
      case Type_Kind (Type_Of (Local)) is
         ...
         when Long_Kind =>
            --  Implicit conversion: if the value that we have in the stack
            --  is the index of an array then it has been previously converted
            --  into an integer and we must undo that conversion here (see
            --  Exp_Ch4.Load_Array_Bounds). Required by the Verifier.

            if Type_Kind (Top_Type) = Int_Kind then
   ...
   begin
      pragma Assert (not Is_Static (Field));
      pragma Assert (Check_Top (Field_Typ));
      pragma Assert (Check_Next (Class_Typ));
   ...
   begin
      ...
      --  NOTE: Not yet implemented (use Gen_New_Multiarray).
         ...
         Error_Msg_N
           ("allocation of multidimensional array not supported", Array_Type);
      end if;
   ...
   is
      pragma Unreferenced (Modular, Integer_Type, Overflow_Check);
   begin
   ...
   procedure Convert_To_Integer (Round : Boolean);
   --  Generates a conversion of the top (integer) stack item to Int

   procedure Convert_To_Long (Round : Boolean);
   ...
   procedure Convert_To_Integer (Round : Boolean) is
   begin
      case Type_Kind (Top (Active_Stack)) is
         when Int_Kind =>
         ...
         when Float_Kind =>
            if Round then
               Gen_Invoke_Static (API_Method (Math_round_float));
            else
               Gen_Instr (F2i);
            end if;
         when Double_Kind =>
            if Round then
               Gen_Invoke_Static (API_Method (Math_round_double));
               Gen_Instr (L2i);
            else
               Gen_Instr (D2i);
            end if;
   ...
   procedure Convert_To_Long (Round : Boolean) is
   begin
      case Type_Kind (Top (Active_Stack)) is
         when Int_Kind =>
            Gen_Instr (I2l);
   ...
   procedure Gen_Box (Target_Type : Type_Id) is
      pragma Unreferenced (Target_Type);
   begin
      null;
   end Gen_Box;
         ...
         when Class_Kind =>
            ...
            then
               ...
               while Super_Class /= Target_Class
                 and then Super_Class /= Java_Lang_Object
                 and then Super_Class /= Null_Class
               loop
                  Super_Class := Superclass (Super_Class);
               end loop;

               if Super_Class /= Null_Class then
   ...
   procedure Gen_Comparison (Unordered : Boolean) is
   begin
      pragma Assert (Type_Kind (Top (Active_Stack)) =
                     Type_Kind (Next_To_Top (Active_Stack)));
   ...
   procedure Check_Compare_Type is
   begin
      case Type_Kind (Top (Active_Stack)) is
   ...
   begin
      if Result_Type (Active_Method) /= Void_Type then
         ...
         if Return_Value = Null_Local_Var then
            ...
            --  Default initialization of wrapped types. Required by
            --  the JVM verifier

            case JVM.Type_Kind (Result_Type (Active_Method)) is
            ...
            Gen_Store_Local (Local_Var (Current_Method, "_retval"));
         end if;
      end if;
   end Declare_Ret_Val;
   ...
   procedure Initialize is
      ...
      pragma Unreferenced (Predef_Method);
