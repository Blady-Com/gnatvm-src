...
private
   ...
   type thread_mutex_t is record
      opaque   : unsigned_long;
      fastlock : access long;
      lockcnt  : int;
   end record;
   pragma Convention (C, thread_mutex_t);
   --  Note that the fields fastlock and lockcnt are not part of older versions
   --  (such as Xenomai 2.4). However, making the type bigger than what is
   --  needed (for old Xenomai versions) is not problematic because the extra
   --  fields can be considered as unused padding.

   type thread_cond_t is record
      opaque : unsigned_long;
   end record;
