      ...
      Entity_Id standard_exception_type, Int gigi_operating_mode)
{
  ...
  if (flag_debug_instances)
    LINEMAPS_INSTANCE_IDS(line_table) =
      (int*) xcalloc(number_file, sizeof (int));
  for (i = 0; i < number_file; i++)
	...
	= IDENTIFIER_POINTER
	 ...
	 of columns chosen to avoid jumping over the next power of 2.  */
      ...
      if (flag_debug_instances)
        LINEMAPS_INSTANCE_IDS(line_table)[i] = file_info_ptr[i].Instance;
  ...
  switch (attribute)
      ...
      else if (TREE_CODE (TREE_TYPE (gnu_prefix)) == FUNCTION_TYPE)
	{
	  ...
	  if (TREE_CODE (gnu_expr) == ADDR_EXPR
	    ...
	    {
	      const int use_runtime_descriptors
#ifdef USE_RUNTIME_DESCRIPTORS
