...
package body System.BB.Interrupts is
   ...
   begin
      ...
      Board_Support.Install_Interrupt_Handler (Interrupt_Wrapper'Address, Id);
   end Attach_Handler;
   ...
   begin
      if Result /= No_Interrupt
        and then Threads.Thread_Self.Active_Priority =
                   Board_Support.Priority_Of_Interrupt (Result)
      then
         --  This CPU is currently executing an interrupt. This is the case if
         --  an interrupt is being handled (obviously), and if the current task
         --  is at the priority of the interrupt (i.e. a higher priority task
         --  is currently running suspending the handling of the interrupt).
   ...
   begin
      --  Handle spurious interrups, reported as No_Interrupt. If they must be
