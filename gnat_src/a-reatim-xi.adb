...
package body Ada.Real_Time is
   ...
   --  not available. Is this really right, overflows will be ignored ???
   ...
   begin
      ...
      if (((Left > 0 and then Right > 0) or else (Left < 0 and then Right < 0))
             and then
               Max_Pos_Time_Span / Uint_64 (abs (Right)) <
                                                     Uint_64 (abs (Left)))
        or else
         (((Left > 0 and then Right < 0) or else (Left < 0 and then Right > 0))
             and then
               Max_Neg_Time_Span / Uint_64 (abs (Right)) <
                                                     Uint_64 (abs (Left)))
      then
   ...
   begin
      ...
      --  are 32-bit integers. Note that the intermediate value is on 96 bits.

      --  It is possible to take the absolute value of TS, as in case of
      --  overflow (which are suppressed), the value will be correct when
      --  converted to Uint_64.

      Result := LLI
        (Mul_Div (Uint_64 (abs TS), Duration_Units, OSI.Ticks_Per_Second));
